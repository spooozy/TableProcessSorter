...
void Sorter::WriteFilesToFiles(const std::vector<FileInfo>& files, int numParts) {
    ...
    for (int i = 0; i < numParts; ++i) {
        std::wstring fileName = L"D:\\ÁÃÓÈÐ\\3 êóðñ\\ÑÏ\\TableProcessSorter\\tempFiles\\temp_file_" + std::to_wstring(i + 1) + L".txt";
...
void Sorter::ProcessCompleted(HANDLE processHandle, int index, int sortCase) {
    std::wstring fileName = L"D:\\ÁÃÓÈÐ\\3 êóðñ\\ÑÏ\\TableProcessSorter\\tempFiles\\temp_file_" + std::to_wstring(index) + L".txt";
    std::wifstream inFile(fileName);
    inFile.imbue(std::locale(""));
...
void Sorter::sort(int sortCase) {
    ...
    for (int i = 0; i < procNum; ++i) {
        ...
        std::wstring command = L"D:\\ÁÃÓÈÐ\\3 êóðñ\\ÑÏ\\TableProcessSorter\\x64\\Debug\\SortingProcess.exe " + std::to_wstring(i + 1) + L" " + std::to_wstring(sortCase);
